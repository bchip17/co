...
async function main() {
  ...
  */
  ...
  // BCP, USDC mock tokens (local only)

  const MockToken = await hre.ethers.getContractFactory("MockToken");
  const bcp = await MockToken.deploy("Bcp", "BCP", 18);
  await bcp.deployed();

  // const bcp = {address: '0x0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF'};
  ...
  // Rewards for Bcp
  const bcpRewardsETH = await Rewards.deploy(poolBCP.address, ADDRESS_ZERO);
  await bcpRewardsETH.deployed();
  console.log("bcpRewardsETH deployed to:", bcpRewardsETH.address);
  ...
  await router.setBcpRewards(ADDRESS_ZERO, bcpRewardsETH.address);
  ...
  await bcpRewardsETH.setRouter(router.address);
