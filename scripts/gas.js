...
async function main() {
  ...
  */
  ...
  // WETH, BCP, USDC mock tokens (local only)
  ...
  const bcp = await MockToken.deploy("Bcp", "BCP", 18);
  await bcp.deployed();

  // const bcp = {address: '0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF'};
  ...
  // Rewards for Bcp
  const bcpRewardsWETH = await Rewards.deploy(poolBCP.address, weth.address);
  await bcpRewardsWETH.deployed();
  console.log("bcpRewardsWETH deployed to:", bcpRewardsWETH.address);
  ...
  await treasury.setBcpPoolShare(weth.address, 1000);
  await treasury.setBcpPoolShare(usdc.address, 1000);
  console.log("set Bcp shares for treasury");
  ...
  await router.setBcpRewards(weth.address, bcpRewardsWETH.address);
  ...
  await bcpRewardsWETH.setRouter(router.address);
  ...
  // await bcp.mint(parseUnits("1000"));
