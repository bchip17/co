...
async function main() {
  ...
  const darkOracleAddress = process.env.LOCAL_DARK_ORACLE_ADDR;
  ...
  const routerAddress = process.env.TESTER_ROUTER_ADDR;
  ...
  // const bcpAddress = '0x292E27B2b439Bb485265aBA27c131247B13593c1';
  // const bcp = await (await ethers.getContractFactory("MockToken")).attach(bcpAddress);
  ...
  // const bcpPool = await (await ethers.getContractFactory("PoolBCP")).attach(await router.bcpPool());

  // const bcpRewardsETH = await (await ethers.getContractFactory("Rewards")).attach(await router.getBcpRewards(usdcAddress));
  // const bcpRewardsUSDC = await (await ethers.getContractFactory("Rewards")).attach(await router.getBcpRewards(usdcAddress));
  
  // console.log('Contracts set', router.address);

  // // Mint some BCP, USDC
  // await usdc.connect(user2).mint(parseUnits("100000", 6));
  // await bcp.connect(user2).mint(parseUnits("1000", 18));
  ...
  // // BCP: stake, unstake, claim rewards
  ...
  // console.log('pool WETH params', (await poolWETH.maxDailyDrawdown()).toString(), (await poolWETH.minDepositTime()).toString(), (await poolWETH.utilizationMultiplier()).toString(), (await poolWETH.maxBcp()).toString(), (await poolWETH.withdrawFee()).toString());
  ...
  // console.log('poolUSDC params', (await poolUSDC.maxDailyDrawdown()).toString(), (await poolUSDC.minDepositTime()).toString(), (await poolUSDC.utilizationMultiplier()).toString(), (await poolUSDC.maxBcp()).toString(), (await poolUSDC.withdrawFee()).toString());
